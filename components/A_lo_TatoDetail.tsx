...
const A_lo_TatoDetail: React.FC = () => {
  ...
  const [article, setArticle] = useState<AlotatoArticle | null>(null);
  ...
  return (
    <main className="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
      <article className="prose max-w-none">
        ...
        <h1 className="text-4xl font-bold text-gray-900 leading-tight mb-6">{article.titular}</h1>
        ...
        <div className="text-gray-700 leading-relaxed">
          {article.cuerpo.map((block, index) => (
            <p key={index} className="mb-6 leading-relaxed">{block.text}</p>
          ))}
        </div>
        <p className="text-lg text-gray-700 italic mt-6">{article.cierre}</p>
      </article>
    </main>
  );
};
